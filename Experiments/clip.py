...
model_name = sys.argv[1] # enter model name, e.g., "openai/clip-vit-base-patch32", etc.
...
train_dataset = MultimodalDataset(train_df, "../Data", processor)
val_dataset = MultimodalDataset(val_df, "../Data", processor)
test_dataset = MultimodalDataset(test_df, "../Data", processor)
...
class CustomCLIPClassifier(nn.Module):
    def __init__(self):
        ...
        for name, param in self.clip.vision_model.named_parameters():
            if any(f"encoder.layers.{i}." in name for i in [-1, -2]):
...
model = CustomCLIPClassifier().to(device)
